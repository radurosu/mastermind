...
public class mmsTester2{
    ...
    public static void main(String[] args)throws java.io.IOException{
        ...
        mmsTester2 solver = new mmsTester2();
        solver.runSolver();
    }//end main

    public mmsTester2(){ } //constructor
    ...
    public void solve()
    {
        ...
        while(!solved)
        {
           ...
           /*if( (guess[0] == guess[1]) &&(guess[0] == guess[2])&&
                    (guess[0] == guess[3])  && (guessNumber ==1))*/
            if(guessNumber == 1 && hasTwo())
                continue;

            if(( guessNumber == 2) && ((board[4][1] + board[5][1])<2)){
                if(!verSecGuess())
                    continue;
            }
            else if(!verifyGuess())//if guess is not ok
...
*/
    ...
    public boolean verSecGuess(){
//        int validNums = new int[4];

        /*  */
        for(int i = 0; i<4; i++)
           for(int j = 0; j<4; j++)
              if((guess[i] == board[j][1]) )
                  return false;
        if(hasThree()){
            return false;}
        return true;
    ...
    public boolean hasTwo(){
           int twos[] = {0,0,0,0,0,0,0};

        for (int i =0; i<4 ; i++)
            twos[guess[i]]++;

        for (int  j =0; j<6; j++)
            if(twos[j] >=2){ //System.out.println("got one");
                return true;}
        return false;
    }
    public boolean hasThree(){ //returns true if the guess has three of one color.
        int threes[] = {0,0,0,0,0,0,0};

        for (int i =0; i<4 ; i++)
            threes[guess[i]]++;

        for (int  j =0; j<6; j++)
            if(threes[j] >=3){ //System.out.println("got one");
                return true;}
        return false;
    }
